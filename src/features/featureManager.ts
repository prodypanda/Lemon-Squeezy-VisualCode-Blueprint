...
import { FeatureResult } from '../types'; // Import FeatureResult
    ...
    public static async executeFeature(feature: string, isPremiumEnabled: boolean): Promise<FeatureResult> {
        try {
            if (CONFIG.FEATURES.PREMIUM.includes(feature) && !isPremiumEnabled) {
                return { success: false, message: CONFIG.UI.MESSAGES.LICENSE_REQUIRED };
            }

            switch (feature) {
                case 'characterCount':
                    return { success: true, message: `Character count: ${FreeFeatures.getCharacterCount()}` };
                case 'wordCount':
                    return { success: true, message: `Word count: ${FreeFeatures.getWordCount()}` };
                case 'toUpperCase':
